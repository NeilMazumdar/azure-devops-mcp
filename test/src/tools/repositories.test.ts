...
describe("repos tools", () => {
  ...
  describe("repo_list_pull_request_threads", () => {
    ...
    it("should list pull request threads with file context", async () => {
      configureRepoTools(server, tokenProvider, connectionProvider, userAgentProvider);
      const call = (server.tool as jest.Mock).mock.calls.find(([toolName]) => toolName === REPO_TOOLS.list_pull_request_threads);
      if (!call) throw new Error("repo_list_pull_request_threads tool not registered");
      const [, , , handler] = call;
      const mockThreads = [
        {
          ...
          lastUpdatedDate: "2023-01-01T11:00:00Z",
          status: CommentThreadStatus.Active,
          threadContext: {
            filePath: "src/main.ts",
            rightFileStart: { line: 42, offset: 5 },
            rightFileEnd: { line: 45, offset: 20 },
          },
          comments: [
            {
              id: 2,
              ...
              publishedDate: "2023-01-01T10:00:00Z",
              isDeleted: false,
              lastUpdatedDate: "2023-01-01T10:15:00Z",
              lastContentUpdatedDate: "2023-01-01T10:05:00Z",
            },
          ],
        },
      ];
